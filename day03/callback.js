...
//     //callback에 함수를 전달했다면 if문으로 , undefined 
//     if(callback){
...
//대소비교
// 두 정수 입력시 첫번째 정수가 크거나 같으면 True 아니면 False이다.
// 큰 값을 다른 함수에서 출력한다.
//정수, boolean, string 중 타입
let result = isBigger(1,2,function(number1, number2, result){
    if(!result){
        return number2;
    }
    if(number1 == number2){
